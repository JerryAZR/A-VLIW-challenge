...
from ir import Sym, Reg, LaneRef, Load, Gather
...
class RenameEngine:
    ...
    def rename(self, instrs: list) -> list:
        """Symbolic -> resolved, single forward pass. For every
        instruction: map its read operands, then its write operands (reads
        first - a self-read-write instruction must see the OLD home on its
        reads), and rebuild it resolved. Gathers are decomposed into VLEN
        scalar loads on the resolved homes (lane addresses are plain
        arithmetic)."""
        out = []
        # backward scan to determine liveliness of temps.
        # Then forward scan to rename, following hints generated by backward scan.
        for instr in instrs:
    ...
    # -- draft, next step: explicit Free IR + auto-free pass -------------
    # def _auto_free(instrs: list):
    #     live_set{}
    #     for instr in instrs.reversed():
    #         for wo in instr.write_operands():
    #             live_set.remove(wo)
    #         for ro in instr.read_operands():
    #             if ro not in live_set:
    #                 live_set.add(ro)
    #                 instrs.insert_after(instr, Free(ro))
