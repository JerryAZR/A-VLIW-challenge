...
`tools/_check_big.py` is a correctness testbench that grows the scratch file
(patching `SCRATCH_SIZE` in both the allocator's pool sizing and the
simulator's per-core scratch array, `python -m tools._check_big [size]`, default
...
by `problem.py`; the patch is validation-only. `tools/_check.py` remains the
